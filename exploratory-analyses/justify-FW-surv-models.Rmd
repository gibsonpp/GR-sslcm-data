...
subtitle: "GR-sslcm Supplemental Analysis"
...
output:
  ...
  # bookdown::word_document2:
    # reference_docx: "C:/Users/bstaton/Desktop/Staton/3_general/Rmd-docx-template/Rmd-docx-template-update.docx"
params:
  scenario:
    value: "base-vshort"
...
knitr::opts_chunk$set(echo = FALSE, fig.align = "center",
                      fig.width = 6 * ifelse(knitr::is_html_output(), 1, 0.75),
                      fig.height = 5 * ifelse(knitr::is_html_output(), 1, 0.75),
                      warning = FALSE, message = FALSE)
if (!knitr::is_html_output()) knitr::opts_chunk$set(dpi = 300)
knitr::opts_knit$set(root.dir = "../../GR-sslcm")
```
...
## most other data is combined survival/length across many fish

# read in the raw individual outcome data set
dat = read.csv("../GR-sslcm-data/exploratory-analyses/parr-summer-tags-lengths-detections.csv")
...
```{r set-colors}
solid = c(data = "royalblue", model = "salmon")
tranp = scales::alpha(solid, 0.35); names(tranp) = names(solid)
```
...
plot_fun = function(pop_keep, dat, x_var, y_var, y_fun = function(y) {y}, y_ifun = function(iy) {iy}) {
  ...
  polygon(c(pred_x, rev(pred_x)), c(lwr, rev(upr)), col = tranp["model"], border = NA)
  lines(mn ~ pred_x, lwd = 3, col = solid["model"])
  lines(lwr ~ pred_x, col = solid["model"])
  lines(upr ~ pred_x, col = solid["model"])
  
  # draw the data points
  points(y_var ~ x_var, data = plot_dat, pch = 21, cex = 1.5,
         col = solid["data"], bg = tranp["data"])
...
:::{.note}
**NOTE**: due to inter-annual variability of capture timing for summer sampling, the mean length data required standardization for additional growth incurred in years of later sampling; all summer mean length data used in the analyses that follow use the standardized version. See the [Appendix](#appendix) for more details.
:::
...
:::{.note}
**NOTE:** Only data for brood years 2000 and later were included in this analysis because these were most easily accessible from PTAGIS; the same is true for Analysis [#5](#a5).
:::
...
plot_f = function(pop_keep) {
  ...
  junk = sapply(unique(plot_dat$brood_year), function(y) lines(surv ~ length, data = subset(plot_dat, brood_year == y), col = tranp["model"]))
}
...
kbl = round(do.call(rbind, eff_summary), 2) %>%
  kable(ifelse(is_html_output(), "html", "pandoc"), col.names = c("Years", "Mean", "Min", "Max", "% > 1"), align = "ccccc",
...
Given this, we wished to determine if the population-level estimates of survival correlate with the average length of the sampled population (tagged and untagged), as should be expected from the findings of Analysis [#2](#a2).
...
Some fish are too small for PIT tagging (<55mm fork length) -- this average size includes the length of all fish captured, not only those that received tags.
...
kbl = eff %>%
  kable(ifelse(is_html_output(), "html", "pandoc"), digits = 2, col.names = c("Odds Ratio per 5mm Increase"), caption = "Population-specific mean length effect on survival from summer tagging to LGR, expressed as the odds ratio for every 5mm increase in mean length (i.e., _for every 5mm increase in length, fish were [ODDS RATIO] times as likely to survive_).")
...
If so, this may indicate that accounting for post-parr recruitment density-dependent dynamics using only mean length may be insufficient.
...
The sole predictor was the reconstructed parr variable described in Analysis [#1](#a1).
We used four separate models, one per population.
...
A complete handling of this analysis would involve re-fitting CJS models with a length covariate so inter-annual variability in detectability is accounted for; we felt that effort was not warranted for this exploratory analysis.
:::
...
_<u>Implications for LCM</u>_ -- Although we cannot entirely exclude the possibility of non-size-mediated density dependent mechanisms post-parr recruitment, our findings gives us confidence that we are not grossly under-accounting the effect of density-dependence by using through size alone.
...
plot_fn = function(df) {
  ...
  cex = 2.1
  par(mfrow = c(3,2), mar = c(2.5,2.5,1,1), mgp = c(1.5,0.25,0), tcl = -0.15, cex.axis = 1.2, cex.lab = 1.2, lend = "square")
  ...
  legend("topright", title = "Length Type", legend = c("Observed", "Standardized"), col = solid[c("data", "model")], bty = "n", pch = 15, pt.cex = 2)
  label("(e)")
...
```{r CAT-len-adj, fig.width = 5, fig.height = 6.75, fig.cap = "Summer mean length data standardization for Catherine Creek. See list above for the interpretation of each panel."}
plot_fn(subset(tmp, population == "CAT"))
```

### LOS

```{r LOS-len-adj, fig.width = 5, fig.height = 6.75, fig.cap = "Summer mean length data standardization for Lostine River. See list above for the interpretation of each panel."}
plot_fn(subset(tmp, population == "LOS"))
```

### MIN

```{r MIN-len-adj, fig.width = 5, fig.height = 6.75, fig.cap = "Summer mean length data standardization for Minam River. See list above for the interpretation of each panel."}
plot_fn(subset(tmp, population == "MIN"))
```

### UGR

```{r UGR-len-adj, fig.width = 5, fig.height = 6.75, fig.cap = "Summer mean length data standardization for Upper Grande Ronde. See list above for the interpretation of each panel."}
